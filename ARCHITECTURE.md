...
> Обновлено 2026-08-27: единая последовательность этапов — MVP → Этап 2 (правила встраивания медиа-объектов) → Этап 3 (админка); добавлены §10-12.
...
2. **Этап 2 — управление правилами встраивания медиа-объектов** — реестр становится местом, где
   ...
   настройки (см. §10, `MEDIA_INTEGRATION.md`).
3. **Этап 3 — развёртывание админки** — UI реестра встраивается точкой входа в единую админку
   компании (см. §11). Может идти параллельно с интеграцией через Splitter BFF (развилка
   MS Catalog vs BFF, §12) и с подключением стейкхолдеров для поставки кандидатов на встраивание
   (`MEDIA_INTEGRATION.md` §9) — все три трека независимы друг от друга после готовности Этапа 2.
...
ранжирование и выдачу контента самостоятельно — реестр не в рантайм-пути формирования выдачи на
MVP.
...
| Эндпоинт отдачи правил встраивания | часть `admin`/`internal`-API реестра | Этап 2 | реестр начинает отдавать правила встраивания потребителю (MS Catalog или Splitter BFF) — см. §10, §12 |
...
routers/          # HTTP-слой: admin / internal / public / (widgets — не нужен реестру)
  ...
  internal/        — MVP: приём входящих данных от RecSys (push по расписанию/триггеру).
                     Этап 2: отдача правил встраивания потребителю (MS Catalog/Splitter BFF)
...
> взаимодействует (отдача правил встраивания потребителям — отдельная линия развития, Этап 2,
...
6. **Оценка Этапа 2 (правила встраивания).** Оценка 26-30 дней (п.4) дана до появления требований
   ...
   см. `TASKS.md`, Этап 2. Отдельно попросили отметить, какие пункты можно распараллелить между
...
8. **Точка входа админки (Этап 3)** — единая админка или отдельный SPA. Для оценки нужны
   фронтенд-ресурсы, которых пока нет в команде (встреча 2026-08-27).


## 10. Правила встраивания медиа-объектов (Этап 2)

Второй этап развития реестра после MVP (учёт плейсментов): управление правилами встраивания
промотируемых объектов стейкхолдеров в рекомендательные листинги. Инициатива зафиксирована
отдельно от учётного MVP (см. `MEDIA_INTEGRATION.md`, v0.4, 2026-08-07 → 2026-08-27) и требует
отдельной оценки (см. `TASKS.md`, Этап 2).
...
## 11. Развёртывание админки (Этап 3)
...
Этап 3 может идти параллельно с интеграцией через Splitter BFF (развилка MS Catalog vs BFF,
§12) и с подключением стейкхолдеров для поставки кандидатов на встраивание
(`MEDIA_INTEGRATION.md` §9) — все три трека независимы друг от друга и не пересекаются по
ресурсам с фронтенд-разработкой админки.

Задачи по этому этапу — `TASKS.md`, раздел «Этап 3». Оценка не дана: по итогам встречи
...
| Влияние на срок | Учтено в текущей оценке 26-30 дней (MVP, см. §9 п.4) — Этап 2 в любом случае требует отдельного пересчёта | Не оценено - доп. работа Splitter + миграция МП, вероятно больше |
...
плейсментов. Требуется отдельная встреча с командами Splitter BFF и LEGO — ожидается на следующей неделе.
