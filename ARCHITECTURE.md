...
Эволюция сервиса (этапность сквозная):

1. **Этап 1. MVP**. Учёт плейсментов рекомендаций: реестр, просмотр конфигурации, распределение плейсментов по экранам. Из внешних
   источников — только RecSys: RecSys сам по расписанию/триггеру
   (например, появился новый `placement_id`) присылает данные о своих плейсментах в реестр.
   Реестр на MVP ничего не отдаёт наружу. Служит справочником.
...
2. **Этап 2. Развёртывание админки** - UI реестра встраивается точкой входа в единую админку
   (см. §11), сразу после готовности сервиса на MVP.
3. **Этап 3. Управление правилами встраивания медиа-объектов** — реестр становится местом, где
   ...
   Контент, Рецепты, Трейд-промо) в рекомендательные листинги: приоритет
   ...
   стейкхолдеров и сборка правил в MS Catalog/Splitter BFF — последний шаг,
   идёт после этого этапа либо параллельно с ним.

Единственный источник плейсментов для реестра - RecSys.

Разделение ответственности: **реестр плейсментов учитывает и описывает плейсменты и
конфигурацию**, RecSys - продолжает отвечать за ранжирование и выдачу контента самостоятельно.

## 2. Технологический стек

Стек выровнен с сервисом MS Catalog, чтобы переиспользовать корпоративную
...
| Очереди/фон. задачи | `arq` (Redis-backed) | Перспектива | на MVP не нужен: push от RecSys обрабатывается синхронно в request-хендлере; нужен в перспективе для периодического опроса AB-платформы |
...
| Аутентификация | X5ID (через `x5d-web` `X5IdClient`) + request signing middleware | MVP | сама аутентификация нужна с MVP; роль «Владелец полки» - в перспективе (см. §7) |
...
| Клиент AB-платформы | синхронизация `ab_status` через `arq`-воркер | Перспектива | без Kafka - по расписанию/триггеру |
| Брокер событий | Kafka (`aiokafka` / Faust-консьюмеры) | Перспектива | подключается, когда появятся реальные event-driven интеграции (см. §6) |
...
`arq`-воркеры по расписанию/триггеру. Kafka подключается в перспективе, когда появятся:
...
- Роли (бизнес-уровень, поверх X5ID): **Администратор реестра** (команда Каталог — полный
  ...
  Владельца полки - в перспективе, при необходимости разграничить доступ по владельцам полок.
...
> Здесь и в §8.2 диаграммы намеренно без API Gateway: реестр ничего не отдаёт потребителям при
> сборке выдачи, поэтому Gateway с реестром не взаимодействует (отдача правил встраивания
> потребителям - отдельная линия развития, Этап 3, см. §10, §12). Эти же диаграммы дублируются
> в `diagrams/*.drawio` с ортогональными линиями без пересечения блоков - Mermaid здесь
> оставлен для быстрого просмотра в GitHub.
...
### 8.2 Контейнеры (Container) сервиса Placement Registry

```mermaid
C4Container
  title Контейнеры — Placement Registry

  Person(user, "Администратор реестра")
  ...
  System_Boundary(pr, "Placement Registry") {
    ...
    Container(worker, "Sync worker", "Python / arq", "MVP: обработка входящих discovery-событий. В перспективе: опрос AB-платформы")
  ...
  System_Ext(ab, "AB-платформа", "В перспективе")
  ...
  Rel(worker, ab, "Опрос статусов A/B-тестов (в перспективе)", "HTTPS/JSON")
...
1. **C4-диаграммы** - готовы Context MVP (§8.1) и Container (§8.2); эти диаграммы
   продублированы как `.drawio`-файлы с ортогональными/закруглёнными линиями (без
   пересечения блоков) в `diagrams/context-mvp.drawio`, `diagrams/container.drawio` - для
   открытия в draw.io и переноса в корпоративный стандарт.
...
4. **Оценка backend** — получена предварительная оценка (2026-06-26, до решения о взятии в работу и выделения ресурсов): анализ/согласование
...
5. **Состав плейсментов RecSys: поиск/листинг** Комментарий RecSys: плейсменты поиска/листинга (4, максимум 6 — по 2 на каждую из 3 торговых
   ...
   RecSys предлагает дождаться перехода RecProcessing с Python на Go.

6. **Оценка Этапа 3 (правила встраивания).** Находится на оценке. Отдельно попросили отметить, какие пункты можно распараллелить между
...
**Статус на 2026-08-27:** решение не принято, но по итогам обсуждений мы больше склоняемся к
варианту 2 (Splitter BFF) как к целевому - миграция МП на Splitter в любом случае в планах
развития мобильного клиента, независимо от реестра плейсментов. Это не финальное решение:
нужно подтверждение ёмкости от команды Splitter и отдельная встреча с командами Splitter BFF и
LEGO. Куда именно стейкхолдеры будут поставлять кандидатов на встраивание (в Catalog или в BFF)
тоже пока открытый вопрос и решается в рамках этой же развилки.
