...
1. **Этап 1. MVP**. Учёт плейсментов рекомендаций: реестр, просмотр конфигурации, карта подборок. Из внешних
...
Единственный источник плейсментов для реестра - RecSys, и в MVP, и далее. Catalog и X5 Media
свои плейсменты в реестр не передают. Отдельная, второстепенная задача - синхронизация статуса
A/B-теста (`ab_status`) с AB-платформой, см. §2.

Разделение ответственности: **реестр плейсментов учитывает и описывает плейсменты и
конфигурацию**, а RecSys - единственный источник данных о них - продолжает отвечать за
