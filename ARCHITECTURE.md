...
> Обновлено 2026-08-27: добавлены §10-12 — правила встраивания медиа-объектов (Этап 3), развёртывание админки в единой админке (Этап 4), развилка MS Catalog vs Splitter BFF.
...
**Вторая линия развития (с 2026-08-07):** параллельно с учётной функцией у реестра появилась вторая задача — управление правилами встраивания промотируемых медиа-объектов в рекомендательные листинги (Этап 3) и последующее размещение админки в контуре единой админки (Этап 4). Это отдельная от пп. 1-3 выше линия развития, требует отдельной оценки — см. §10-12 и `TASKS.md`.
...
| Очереди/фон. задачи | `arq` (Redis-backed) | Этап 2 | на MVP не нужен: push от RecSys обрабатывается синхронно в request-хендлере; нужен с Этапа 2 для периодического опроса AB-платформы |
...
3. **Архитектурное согласование** — по итогам встречи с лидом бэкенда Каталога (2026-06-26):
   разработка скорее всего перейдёт команде Рекомендаций Каталога, деливери — Анастасия
   Силуанова и Владимир Мальцев. Следующий шаг — показать им идею и сервис, затем обсудить с
   лидом разработки Антоном (верхнеуровневый анализ, валидация архитектуры/диаграмм), и только
...
6. **Оценка Этапа 3 (правила встраивания).** Оценка 26-30 дней (п.4) дана до появления требований
   по встраиванию медиа-объектов и покрывает только MVP/учёт. Антону Царенко переданы материалы
...
8. **Точка входа админки (Этап 4)** — единая админка или отдельный SPA. Для оценки нужны
   фронтенд-ресурсы, которых пока нет в команде (встреча 2026-08-27).


## 10. Правила встраивания медиа-объектов (Этап 3)

Вторая функция реестра, помимо учёта плейсментов (MVP) и подключения источников (Этап 2):
управление правилами встраивания промотируемых объектов стейкхолдеров в рекомендательные
листинги. Инициатива зафиксирована отдельно от учётного MVP (см. `MEDIA_INTEGRATION.md`, v0.4,
2026-08-07 → 2026-08-27) и требует отдельной оценки (см. `TASKS.md`, Этап 3).
...
## 11. Развёртывание админки (Этап 4)
...
Задачи по этому этапу — `TASKS.md`, раздел «Этап 4». Оценка не дана: по итогам встречи
...
плейсментов. Требуется отдельная встреча с командами Splitter BFF и LEGO (Юрий Сазанков — в
отпуске на неделе после 2026-08-27, встреча ожидается на следующей неделе).
